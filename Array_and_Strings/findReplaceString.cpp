...
For example, if we have S = "abcd" and we have some replacement operation i = 2, x = "cd", y = "ffff", then because "cd" starts at position 2 in 
...
public:
    ...
    string findReplaceString(string S, vector<int>& indexes, vector<string>& sources, vector<string>& targets) {
        ...
        for(i = 0;i<indexes.size();i++)
            sorted_map.insert(std::pair<int,int>(indexes[i],i));
        ...
        for(auto it =sorted_map.begin();it!=sorted_map.end();it++)
        {
            ...
            {
                target+=S.substr(pos,indexes[i]-pos);
                target+=targets[i];
                pos = indexes[i]+sources[i].size(); // shift the position.
            }
        }
        target+=S.substr(pos);
        return target;

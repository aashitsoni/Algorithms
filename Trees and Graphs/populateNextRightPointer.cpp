...
class Solution {
    vector<Node*> nodeVec;
public:
    void assignRight( Node* node, int level ) {
        node->next = nullptr;
        if( level > nodeVec.size() ) {
            nodeVec.push_back( node );
        } else {
            Node* prev = nodeVec[level-1];
            prev->next = node;
            nodeVec[level-1] = node;
        }
    }
    
    void buildVector( Node* node, int level ) {
        if( node ) {
            assignRight( node, level );
